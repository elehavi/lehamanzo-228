...
case class TPUParams(width: Int) {
    // TODO: parametrize the size of the systolic array
    val w = width
}

class TPU_fixed(TPUParams: p) extends Module {
    val io = IO(new Bundle {
        ...
        val outResult = Decoupled(SInt(p.w.W)) // TODO: this should be a matrix/bunch of ports, not 
    })
    // TODO: Parameterize generation of MACS, hardcode for now
    val macNE = Module(new MAC(MACParams(width))) // inLeft, inTop, outRight, outBottom, outResult
    val macNW = Module(new MAC(MACParams(width)))
    val macSE = Module(new MAC(MACParams(width)))
    val macSW = Module(new MAC(MACParams(width)))
